...
sfr_desc_path <- here::here("data", "variable_descriptions", 
                            "self_report_var_desc.rds")
...
```{r}
#| echo: false
sfr_cat_cols <- sfr %>% select(ends_with("cat_f")) %>% 
...
sfr_cont_cols <- sfr %>% select(ends_with("years"), ends_with("times")) %>% 
  select(order(colnames(.))) %>% names()
```
...
```{r, echo = FALSE}
# Create flextable
cont_flextable <- flextable(sfr_cont_table, col_keys = 
                              c("var", "description",  "n", "formatted_stats")
                            ) %>%
  # Rename columns
  set_header_labels(
    var = "Variable",
