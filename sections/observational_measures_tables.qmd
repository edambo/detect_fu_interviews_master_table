...
obs_path <- here::here("data", "cleaned_rds_files", 
                       "observational_measures_import.rds")
obs <- readRDS(obs_path)

# Import data frame containing description of variables
obs_desc_path <- here::here("data", "variable_descriptions", 
                            "observational_measures_var_desc.rds")
...
obs_cols <- obs %>% select(ends_with("_f")) %>% select(order(colnames(.))) %>% 
  names()
```
...
```{r, include = FALSE}
free_res <- obs_desc %>% filter(grepl("_comments$", name) | 
                                  grepl("at_.+_why", name)) 
```
...
print(obs_doc, target = here::here("sections", 
                                   "observational_measures_tables.docx"))
```
