...
abuse_det <- compare_det(final_det, detect_tool)

#d15a8e #95b0ff

# Plot Confusion matrix
conf_mat <- ggplot(data =  abuse_det, mapping = aes(x = abuse_any, y = detect_tool_det)) +
  xlab("LEAD Panel Assessment Determination") +
  ylab("Initial DETECT Tool Determination") +
  geom_tile(aes(fill = labels), colour = "white") +
  geom_text(aes(label = labels), vjust = 1, fontface  = "bold", color = "white") +
  scale_fill_gradient(low = "#9dcfbc", high = "#7bb0a0")+
...
lead_doc <- lead_doc %>%
  ...
  # Add Unique MedStar ID summary table
  body_add_flextable(width(unique_case_flextable, width = 1.7, unit = "in"), align = "left") %>%
  body_add_par("") %>%
...
# Add final determination tables  
lead_doc <- lead_doc %>%
...
# Add Confusion Matrix Plot to Word file
