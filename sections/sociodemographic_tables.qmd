...
socio_demo_cat_cols <- socio_demo %>% select(ends_with("cat_f")) %>% 
...
socio_demo_cont_cols <- c("sode_age_years", "sode_people_9cat")
```
...
socio_demo_cat_table_list <- split(socio_demo_cat_table, 
                                   socio_demo_cat_table$var)
...
```{r, echo = FALSE}
# Create flextable
cont_flextable <- flextable(socio_demo_cont_table, 
                            col_keys = c("var", "description",  "n", 
                                         "formatted_stats")) %>%
  # Rename columns
  set_header_labels(
    var = "Variable",
...
soc_doc <- soc_doc %>%
  ...
  body_add_flextable(width(cont_flextable, j = c(1,2,3,4), 
                           width = c(1.7, 2.7, 0.7, 1.7), unit = "in"), 
                     align = "left") %>%
  body_add_par("") %>%
...
for (var_name in socio_demo_cat_cols) {
  ...
  # Set width of tables
  cat_flex <- cat_flex %>% width(j = c(1,2,3,4), width = c(2, 2.7, 0.7, 1.4), 
                                 unit = "in") %>%
...
print(soc_doc, target = here::here("sections", 
                                   "sociodemographic_information_tables.docx"))
```
